...
def train_relation_extraction_model(model, train_dataloader, val_dataloader, num_epochs, device, relation_map):
...
def load_model_and_tokenizer(output_dir, num_labels):
    ...
    model = RelationExtractionModel(num_labels=num_labels)
...
if __name__ == '__main__':
    ...
    model = RelationExtractionModel(num_labels=num_labels, freeze_bert_layers=False)

    # Обучение модели
    train_relation_extraction_model(model, train_dataloader, val_dataloader, num_epochs=10, device=device, relation_map=relation_map)
